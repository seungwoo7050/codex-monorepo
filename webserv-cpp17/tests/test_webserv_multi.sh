#!/usr/bin/env bash
# webserv-cpp17 v0.2.0 테스트: 다중 연결 처리와 타임아웃 동작을 검증한다.
set -euo pipefail
...
if ! grep -q "Hello from webserv v0.2.0" "$resp1"; then
  echo "첫 번째 응답 본문이 예상과 다릅니다." >&2
  cat "$resp1" >&2
  exit 1
fi

if ! grep -q "Hello from webserv v0.2.0" "$resp2"; then
...
echo "webserv v0.2.0 다중 연결/타임아웃 테스트 통과"
