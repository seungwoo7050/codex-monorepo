...
- 테스트
  ...
  ```
- 주요 옵션: 단일 입력 라인 기반으로 환경 변수 확장 및 cd/exit/env 빌트인을 제공한다.
...
- 실행 예시: 기본값으로 8080 포트, 최대 처리 요청 3건
  ...
  ```
- select 기반 이벤트 루프를 사용하며, 타임아웃은 약 1.5초로 설정되어 있다.
...
- 중지
  ...
  docker compose down
  ```
- 구성: app(db/redis 의존), db(MariaDB), redis, nginx(8080 포트 프록시) 서비스를 포함한다. Nginx 정적 파일은 `services/nginx/static/`에서 제공된다.
